Describe 'Idempotent WordPress desired state' {
    BeforeAll {
        ...
        $pageContent = (Get-Content -Raw (Join-Path $projectRoot 'data\pages\gioi-thieu.html')) + (Get-Content -Raw (Join-Path $projectRoot 'data\pages\lien-he.html'))
    }
    ...
    It 'loads version-controlled editable page content' { $source | Should -Match 'data\\pages\\gioi-thieu\.html'; $source | Should -Match 'data\\pages\\lien-he\.html'; $source | Should -Match '__ABOUT_CONTENT__'; $source | Should -Match '__CONTACT_CONTENT__' }
